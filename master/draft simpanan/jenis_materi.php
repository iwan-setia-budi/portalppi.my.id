...
if(isset($_POST['action']) && $_POST['action']=='save'){
    ...
    if (!isset($_POST['csrf_token']) || 
        ...
        header("Location: jenis_materi.php?msg=error");
        exit;
    }

    $nama = trim($_POST['nama_materi']);
    $nama = preg_replace('/\s+/', ' ', $nama);

    if($nama === ''){
        header("Location: jenis_materi.php?msg=empty");
        exit;
    }

    if(mb_strlen($nama, 'UTF-8') > 100){
        header("Location: jenis_materi.php?msg=toolong");
        exit;
    }
    ...
    if($result->num_rows > 0){
        ...
        if($row['status'] == 'nonaktif'){
            ...
            header("Location: jenis_materi.php?msg=reactivated");
            exit;

        } else {
            // SUDAH ADA DAN AKTIF
            header("Location: jenis_materi.php?msg=exist");
            exit;
        }
    }
    ...
    // INSERT BARU
    $stmt = $conn->prepare("
        INSERT INTO tb_jenis_materi(nama_materi, status) 
        VALUES (?, 'aktif')
    ");
    $stmt->bind_param("s", $nama);
    $stmt->execute();

    header("Location: jenis_materi.php?msg=success");
    exit;
}
...
if(isset($_POST['action']) && $_POST['action']=='update'){
    ...
    if($result->num_rows == 0){
        header("Location: jenis_materi.php?msg=notfound");
        exit;
    }
    ...
    if($lama['status'] !== 'aktif'){
        header("Location: jenis_materi.php?msg=inactive");
        exit;
    }

    // Kalau tidak ada perubahan
    if($lama['nama_materi'] === $nama){
        header("Location: jenis_materi.php?msg=nochange");
        exit;
    }
    ...
    if($stmt->execute()){
        ...
        header("Location: jenis_materi.php?msg=updated");
        exit;
    }
...
if (!isset($_POST['csrf_token']) || 
    ...
    $stmt = $conn->prepare("UPDATE tb_jenis_materi SET status='nonaktif' WHERE id=?");
...
if($stmt->execute()){
    ...
    header("Location: jenis_materi.php?msg=deleted");
}
...
<head>
    ...
    <title>Master Jenis Materi | PPI PHBW</title>
            ...
            <div class="container-master">
                ...
                <div class="card">
                    ...
                    <form method="post">
                        ...
                        <div style="display:flex; gap:10px;">
                            <input type="text" name="nama_materi" required placeholder="Masukkan Nama Materi">
                ...
                <div class="card">
                    ...
                    <div class="search-box">
                        <input type="text" id="search" placeholder="Cari Materi...">
                    </div>
                    ...
                    <div class="table-wrapper">
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <?= htmlspecialchars($row['nama_materi']); ?>
                                    </td>
                                    <td>
                                        <button class="edit"
                                            onclick="openEditModal('<?= $row['id']; ?>','<?= htmlspecialchars($row['nama_materi']); ?>')">
                                            Edit
                                        </button>
            ...
            <div id="editModal" class="modal">
                <div class="modal-content">
                    ...
                    <form method="post">
                        ...
                        <input type="text" name="nama_materi" id="editNama"
