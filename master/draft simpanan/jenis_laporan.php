...
if(isset($_POST['action']) && $_POST['action']=='save'){
    ...
    if (!isset($_POST['csrf_token']) || 
        ...
        header("Location: jenis_laporan.php?msg=error");
        exit;
    }

    $nama = trim($_POST['nama_laporan']);
    $nama = preg_replace('/\s+/', ' ', $nama);

    if($nama === ''){
        header("Location: jenis_laporan.php?msg=empty");
        exit;
    }

    if(mb_strlen($nama, 'UTF-8') > 100){
        header("Location: jenis_laporan.php?msg=toolong");
        exit;
    }
    ...
    if($result->num_rows > 0){
        ...
        if($row['status'] == 'nonaktif'){
            ...
            header("Location: jenis_laporan.php?msg=reactivated");
            exit;

        } else {
            // SUDAH ADA DAN AKTIF
            header("Location: jenis_laporan.php?msg=exist");
            exit;
        }
    }
    ...
    // INSERT BARU
    $stmt = $conn->prepare("
        INSERT INTO tb_jenis_laporan(nama_laporan, status) 
        VALUES (?, 'aktif')
    ");
    $stmt->bind_param("s", $nama);
    $stmt->execute();

    header("Location: jenis_laporan.php?msg=success");
    exit;
}
...
if(isset($_POST['action']) && $_POST['action']=='update'){
    ...
    if($result->num_rows == 0){
        header("Location: jenis_laporan.php?msg=notfound");
        exit;
    }
    ...
    if($lama['status'] !== 'aktif'){
        header("Location: jenis_laporan.php?msg=inactive");
        exit;
    }

    // Kalau tidak ada perubahan
    if($lama['nama_laporan'] === $nama){
        header("Location: jenis_laporan.php?msg=nochange");
        exit;
    }
    ...
    if($stmt->execute()){
        ...
        header("Location: jenis_laporan.php?msg=updated");
        exit;
    }
...
if (!isset($_POST['csrf_token']) || 
    ...
    $stmt = $conn->prepare("UPDATE tb_jenis_laporan SET status='nonaktif' WHERE id=?");
...
if($stmt->execute()){
    ...
    header("Location: jenis_laporan.php?msg=deleted");
}
...
$pageTitle = "MASTER JENIS LAPORAN";
                ...
                <div class="card">
                    ...
                    <form method="post">
                        ...
                        <div style="display:flex; gap:10px;">
                            <input type="text" name="nama_laporan" required placeholder="Masukkan Nama Laporan">
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="edit"
                                            onclick="openEditModal('<?= $row['id']; ?>','<?= htmlspecialchars($row['nama_laporan']); ?>')">
                                            Edit
                                        </button>
            ...
            <div id="editModal" class="modal">
                <div class="modal-content">
                    ...
                    <form method="post">
                        ...
                        <input type="text" name="nama_laporan" id="editNama"
