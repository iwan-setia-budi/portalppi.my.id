...
$qAudit = mysqli_query($conn, "SELECT * FROM audit_praktik_lumbal_fungsi WHERE id = $id");
...
$existingJawaban = [];
$qDetail = mysqli_query($conn, "SELECT kode_bagian, urutan_item, jawaban FROM detail_audit_praktik_lumbal_fungsi WHERE audit_id = $id");
  ...
  if (!$tanggal || !$petugas || $ruangan === '' || !in_array($ruangan, $ruanganDiauditOptions, true)) {
    ...
    if ($invalid) {
      ...
      if (preg_match('/^data:image\/png;base64,/', $signatureData)) {
        ...
        if ($signatureBinary !== false && strlen($signatureBinary) > 0) {
          ...
          if (file_put_contents($uploadDir . $signatureFileName, $signatureBinary) !== false) {
            $ttd = 'uploads/audit_praktik_lumbal_fungsi/' . $signatureFileName;
          }
        }
      }
      ...
      if ($ttd === '') {
        ...
        try {
          $stmt = mysqli_prepare($conn, "UPDATE audit_praktik_lumbal_fungsi SET tanggal_audit=?, ruangan_diaudit=?, catatan_audit=?, nama_petugas_unit=?, tanda_tangan_petugas=? WHERE id=?");
...
<head>
  ...
  <title>Edit Audit Praktik Lumbal Fungsi</title>
...
<body class="audit-page">
  <div class="layout">
    ...
    <main>
      ...
      <div class="page-wrap">
        <div class="hero-card">
          <h1>Edit Audit Praktik Lumbal Fungsi #<?= (int) $audit['id'] ?></h1>
        ...
        <form method="post">
          ...
          <div class="section-card">
            ...
            <div class="btn-row">
              ...
              <a class="btn btn-secondary" href="../audit_praktik_lumbal_fungsi.php?tab=tab-data">Data Audit</a>
            </div>
          </div>
        </form>
      </div>
