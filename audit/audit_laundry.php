...
if (isset($_GET['hapus_foto_group'])) {
    if (!ppi_audit_delete_allowed()) {
        echo "<script>alert('Hanya administrator yang dapat menghapus data.'); location.href='audit_laundry.php?tab=foto';</script>";
        exit;
    }
...
<?php
    ...
    if (isset($_GET['hapus'])) {
        if (!ppi_audit_delete_allowed()) {
            echo "<script>alert('Hanya administrator yang dapat menghapus data.'); window.location.href='audit_laundry.php?tab=rekap';</script>";
            exit;
        }
        $id = (int) $_GET['hapus'];
