...
$pageTitle = "AUDIT AMBULANCE";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_ambulance_ok'])) {
  unset($_SESSION['flash_audit_ambulance_ok']);
  $message = '<div class="info-box success">Data audit ambulance berhasil disimpan.</div>';
}
...
$checklistSections = [
  'B00' => [
    'title' => 'Checklist Audit Ambulance',
    'items' => [
      'Dilakukan Pembersihan Rutin (ada bukti ceklist kebersihan)',
      'Stercher dibersihkan rutin( ada bukti ceklist kebersihan)',
      'Terdapat fasilitas APD (Sarung tangan, masker, Apron)',
      'Terdapat fasilitas Kebersihan Tangan',
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_ambulance/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_ambulance/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_ambulance (tanggal_audit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_ambulance (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_ambulance_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'amb_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_ambulance/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_ambulance_ok'] = true;
      header('Location: audit_ambulance.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit ambulance.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_ambulance a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  LEFT JOIN detail_audit_ambulance d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_ambulance_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Ambulance | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Ambulance</h1>
          <p class="subtitle">Form checklist B0001-B0006, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_ambulance/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_ambulance/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_ambulance/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_ambulance/tab_form_audit.php';
            break;
        }
        ?>
      </div>
