...
$pageTitle = "AUDIT UNIVERSAL MASKING";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_universal_masking_ok'])) {
  unset($_SESSION['flash_audit_universal_masking_ok']);
  $message = '<div class="info-box success">Data audit universal masking berhasil disimpan.</div>';
}
...
$checklistSections = [
  'L' => [
    ...
    'items' => [
      'Menggunakan masker',
      'Penggunaan masker sesuai',
      'Masker digunakan terus-menerus',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit || $ruanganDiaudit === '' || !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_universal_masking/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_universal_masking/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_universal_masking (tanggal_audit, ruangan_diaudit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_universal_masking (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_universal_masking_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'umk_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_universal_masking/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_universal_masking_ok'] = true;
      header('Location: audit_universal_masking.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit universal masking.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_universal_masking a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_universal_masking a
  LEFT JOIN detail_audit_universal_masking d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_universal_masking a
  JOIN detail_audit_universal_masking d ON a.id = d.audit_id
  $rekapWhereSql
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_universal_masking_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Universal Masking | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Universal Masking</h1>
          <p class="subtitle">Form checklist indikator L0001-L0003 universal masking, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_universal_masking/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_universal_masking/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_universal_masking/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_universal_masking/tab_form_audit.php';
            break;
        }
        ?>
      </div>
