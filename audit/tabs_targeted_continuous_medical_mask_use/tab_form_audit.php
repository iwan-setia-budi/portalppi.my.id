...
<style>
  ...
  /* Jarak vertikal antar grup field (mis. tanggal → label ruangan) */
  #tab-form .section-card .form-control + .field-label,
  #tab-form .section-card textarea.form-control + .field-label {
    margin-top: 20px;
  }
...
<div id="tab-form" class="tab-pane active<?= !empty($auditMarkAllEnabled) ? ' show-bulk-actions' : '' ?>">
  <form method="post" enctype="multipart/form-data">
    <div class="section-card">
      <h2 class="section-title">Form Audit Targeted Continuous Medical Mask Use</h2>
      ...
      <select name="ruangan_diaudit" class="form-control" required>
        ...
        <?php foreach ($ruanganDiauditOptions as $opt): ?>
          <option value="<?= htmlspecialchars($opt) ?>" <?= (($_POST['ruangan_diaudit'] ?? '') === $opt) ? 'selected' : '' ?>><?= htmlspecialchars($opt) ?></option>
        <?php endforeach; ?>
      </select>
