...
$pageTitle = "AUDIT KEWASPADAAN TRANSMISI";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_kewaspadaan_transmisi_ok'])) {
  unset($_SESSION['flash_audit_kewaspadaan_transmisi_ok']);
  $message = '<div class="info-box success">Data audit kewaspadaan transmisi berhasil disimpan.</div>';
}
...
$checklistSections = [
  'J01' => [
    ...
    'items' => [
      'Petugas memahami transmisi penyakit menular',
      'Terdapat poster transmisi di pintu masuk kamar pasien',
      'Pasien di tempatkan di ruang sendiri atau kohort yang bertekanan negatif',
      'Jika kohorting, pasien ditempatkan dengan diagnosa penyakit yang sama',
      'Jarak tempat tidur minimal 1 meter',
      'Pastikan pintu ruangan selalu tertutup untuk transmisi airborne',
  ...
  'J02' => [
    ...
    'items' => [
      'Pasien dan petugas menggunakan masker selama dalam proses transfer',
      'Petugas menghubungi security untuk mengosongkan lift',
      'Petugas mengantar pasien di dalam lift, tidak ada pengunjung yang menyertai pasien.',
      'Diruang pemeriksaan penunjang pasien tidak diantrikan, langsung di lakukan pemeriksaan',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit || $ruanganDiaudit === '' || !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_kewaspadaan_transmisi/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_kewaspadaan_transmisi/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_kewaspadaan_transmisi (tanggal_audit, ruangan_diaudit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_kewaspadaan_transmisi (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_kewaspadaan_transmisi_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_kewaspadaan_transmisi/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_kewaspadaan_transmisi_ok'] = true;
      header('Location: audit_kewaspadaan_transmisi.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit kewaspadaan transmisi.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_kewaspadaan_transmisi a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_kewaspadaan_transmisi a
  LEFT JOIN detail_audit_kewaspadaan_transmisi d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_kewaspadaan_transmisi a
  JOIN detail_audit_kewaspadaan_transmisi d ON a.id = d.audit_id
  $rekapWhereSql
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_kewaspadaan_transmisi_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Kewaspadaan Transmisi | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Kewaspadaan Transmisi</h1>
          <p class="subtitle">Form checklist J01–J02 (penempatan &amp; transfer pasien), data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_kewaspadaan_transmisi/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_kewaspadaan_transmisi/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_kewaspadaan_transmisi/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_kewaspadaan_transmisi/tab_form_audit.php';
            break;
        }
        ?>
      </div>
