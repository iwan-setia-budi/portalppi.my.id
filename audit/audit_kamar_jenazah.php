...
$pageTitle = "AUDIT KAMAR JENAZAH";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_kamar_jenazah_ok'])) {
  unset($_SESSION['flash_audit_kamar_jenazah_ok']);
  $message = '<div class="info-box success">Data audit kamar jenazah berhasil disimpan.</div>';
}
...
$checklistSections = [
  'E01' => [
    'title' => 'Kebersihan dan Keamanan',
    'items' => [
      'Lantai bersih dan tidak licin',
      'Tidak berdebu',
      'Tidak ada lawa-lawa dan bebas jamur',
      'Keranda selalu bersih dan tidak berkarat',
      'Pembersihan kamar jenazah dilakukan setiap hari',
    ]
  ],
  'E02' => [
    'title' => 'Ketersediaan Alat Pelindung Diri',
    'items' => [
      'Tutup kepala',
      'Masker',
      'Kaca mata goggles',
  ...
  'E03' => [
    ...
    'items' => [
      ...
      'Terdapat Spill Kit di Kamar Jenazah',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_kamar_jenazah/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_kamar_jenazah/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_kamar_jenazah (tanggal_audit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_kamar_jenazah (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_kamar_jenazah_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'kj_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_kamar_jenazah/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_kamar_jenazah_ok'] = true;
      ...
      $message = '<div class="info-box error">Gagal menyimpan data audit kamar jenazah.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_kamar_jenazah a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_kamar_jenazah a
  LEFT JOIN detail_audit_kamar_jenazah d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_kamar_jenazah a
  JOIN detail_audit_kamar_jenazah d ON a.id = d.audit_id
  $rekapWhereSql
...
$bagianLabelsExport = [
  'E01' => 'Kebersihan dan Keamanan',
  'E02' => 'Ketersediaan Alat Pelindung Diri',
  'E03' => 'Fasilitas',
];
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_kamar_jenazah_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Kamar Jenazah | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Kamar Jenazah</h1>
          <p class="subtitle">Form checklist E01–E03 (kebersihan, APD, fasilitas), data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_kamar_jenazah/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_kamar_jenazah/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_kamar_jenazah/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_kamar_jenazah/tab_form_audit.php';
            break;
        }
        ?>
      </div>
