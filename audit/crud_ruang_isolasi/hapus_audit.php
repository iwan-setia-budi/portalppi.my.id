...
ppi_require_admin_delete_redirect('../audit_ruang_isolasi.php?tab=tab-data');

mysqli_begin_transaction($conn);
try {
  mysqli_query($conn, "DELETE FROM audit_ruang_isolasi_foto WHERE audit_id = $id");
  mysqli_query($conn, "DELETE FROM detail_audit_ruang_isolasi WHERE audit_id = $id");
  mysqli_query($conn, "DELETE FROM audit_ruang_isolasi WHERE id = $id");
  mysqli_commit($conn);
} catch (Throwable $e) {
