<style>
  ...
  #tab-rekap .filter-grid { display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; align-items: center; }
  #tab-rekap .stats-grid { display:grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; margin-top: 12px; }
  #tab-rekap .stat-box {
...
if (isset($checklistSections) && is_array($checklistSections)) {
  foreach ($checklistSections as $kode => $section) {
    ...
    foreach ($items as $idx => $itemText) {
      $indikatorKode = (string) $kode . str_pad((string) ($idx + 1), 2, '0', STR_PAD_LEFT);
...
if (!empty($filterRekapSubbab) && isset($checklistSections[$filterRekapSubbab])) {
  $kodeUrutan = [];
  foreach (($checklistSections[$filterRekapSubbab]['items'] ?? []) as $idx => $_t) {
    $kodeUrutan[] = $filterRekapSubbab . str_pad((string) ($idx + 1), 2, '0', STR_PAD_LEFT);
  }
}
if (count($kodeUrutan) === 0) {
  $kodeDariData = array_map(static function ($row) {
    return (string) ($row['kode_bagian'] ?? '');
  }, $rekapRows);
  $kodeUrutan = array_values(array_filter(array_unique($kodeDariData), static function ($v) {
    return $v !== '';
  }));
}
sort($kodeUrutan);
...
<div id="tab-rekap" class="tab-pane active">
  <div class="section-card">
    ...
    <form method="get">
      ...
      <div class="filter-grid">
        ...
        <select name="rekap_ruangan" class="form-control" title="Filter berdasarkan ruangan yang diaudit">
          <option value="">Semua ruangan</option>
          <?php foreach ($ruanganDiauditOptions as $optRu): ?>
            <option value="<?= htmlspecialchars($optRu) ?>" <?= (string) ($filterRekapRuangan ?? '') === $optRu ? 'selected' : '' ?>><?= htmlspecialchars($optRu) ?></option>
          <?php endforeach; ?>
        </select>
        <select name="rekap_subbab" class="form-control" title="Filter berdasarkan sub bab">
          <option value="">Semua sub bab</option>
          <?php foreach ($checklistSections as $sk => $ssec): ?>
            <option value="<?= htmlspecialchars($sk) ?>" <?= (string) ($filterRekapSubbab ?? '') === $sk ? 'selected' : '' ?>><?= htmlspecialchars($sk . ' — ' . ($ssec['title'] ?? '')) ?></option>
          <?php endforeach; ?>
        </select>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-box">
        ...
        <div class="stat-value" style="font-size:18px;line-height:1.25;"><?= !empty($filterRekapRuangan) ? htmlspecialchars($filterRekapRuangan) : 'Semua' ?></div>
      </div>
    </div>
  </div>
  ...
  <div class="section-card" id="rekap-bagian-card">
    ...
    <div class="download-toolbar">
      <a class="btn-download-image js-download-image" href="#" data-target-id="rekap-bagian-card" data-file-prefix="rekap_bagian_audit_unit">Download Gambar Rekap Bagian</a>
    </div>
  ...
  <div class="section-card" id="rekap-periode-card">
    ...
    <div class="download-toolbar">
      <a class="btn-download-image js-download-image" href="#" data-target-id="rekap-periode-card" data-file-prefix="rekap_periode_audit_unit">Download Gambar Rekap Periode</a>
    </div>
...
<script>
  (function () {
    ...
    function renderTableDataToCanvas(target) {
      ...
      const title = titleEl ? titleEl.textContent.trim() : 'Rekap Audit Unit';
      if (!tableEl) {
    ...
    document.querySelectorAll('.js-download-image').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        const filePrefix = btn.getAttribute('data-file-prefix') || 'rekap_audit_unit';
