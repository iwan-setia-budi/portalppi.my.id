...
$grafikSubbab = isset($_GET['grafik_subbab']) ? (string) $_GET['grafik_subbab'] : '';
if ($grafikSubbab !== '' && (!isset($checklistSections) || !isset($checklistSections[$grafikSubbab]))) {
  $grafikSubbab = '';
}

$grafikRuangan = isset($_GET['grafik_ruangan']) ? (string) $_GET['grafik_ruangan'] : '';
if ($grafikRuangan !== '' && (!isset($ruanganDiauditOptions) || !in_array($grafikRuangan, $ruanganDiauditOptions, true))) {
  $grafikRuangan = '';
}
...
if ($grafikRuangan !== '') {
  $grafikWhere[] = "a.ruangan_diaudit = '" . mysqli_real_escape_string($conn, $grafikRuangan) . "'";
}
if ($grafikSubbab !== '') {
  $grafikWhere[] = "d.kode_bagian = '" . mysqli_real_escape_string($conn, $grafikSubbab) . "'";
}
...
if ($grafikRuangan !== '') {
  $trendWhere[] = "a.ruangan_diaudit = '" . mysqli_real_escape_string($conn, $grafikRuangan) . "'";
}
if ($grafikSubbab !== '') {
  $trendWhere[] = "d.kode_bagian = '" . mysqli_real_escape_string($conn, $grafikSubbab) . "'";
}
...
<style>
  ...
  #tab-grafik .filter-grid { display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; align-items: center; }
  ...
  <div class="section-card">
    ...
    <form method="get">
      ...
      <div class="filter-grid">
        ...
        <select name="grafik_ruangan" class="form-control" title="Filter berdasarkan ruangan yang diaudit">
          <option value="">Semua ruangan</option>
          <?php foreach ($ruanganDiauditOptions as $optGr): ?>
            <option value="<?= htmlspecialchars($optGr) ?>" <?= $grafikRuangan === $optGr ? 'selected' : '' ?>><?= htmlspecialchars($optGr) ?></option>
          <?php endforeach; ?>
        </select>
        <select name="grafik_subbab" class="form-control" title="Filter grafik per sub bab">
          <option value="">Semua sub bab</option>
          <?php foreach ($checklistSections as $gk => $gsec): ?>
            <option value="<?= htmlspecialchars($gk) ?>" <?= $grafikSubbab === $gk ? 'selected' : '' ?>><?= htmlspecialchars($gk . ' — ' . ($gsec['title'] ?? '')) ?></option>
          <?php endforeach; ?>
        </select>
