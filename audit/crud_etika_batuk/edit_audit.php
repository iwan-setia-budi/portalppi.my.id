...
$qAudit = mysqli_query($conn, "SELECT * FROM audit_etika_batuk WHERE id = $id");
...
$existingJawaban = [];
$qDetail = mysqli_query($conn, "SELECT kode_bagian, urutan_item, jawaban FROM detail_audit_etika_batuk WHERE audit_id = $id");
  ...
  if (!$tanggal || !$petugas || $ruangan === '' || !in_array($ruangan, $ruanganDiauditOptions, true)) {
    ...
    if ($invalid) {
      ...
      if (preg_match('/^data:image\/png;base64,/', $signatureData)) {
        ...
        if ($signatureBinary !== false && strlen($signatureBinary) > 0) {
          ...
          if (file_put_contents($uploadDir . $signatureFileName, $signatureBinary) !== false) {
            $ttd = 'uploads/audit_etika_batuk/' . $signatureFileName;
          }
        }
      }
      ...
      if ($ttd === '') {
        ...
        try {
          $stmt = mysqli_prepare($conn, "UPDATE audit_etika_batuk SET tanggal_audit=?, ruangan_diaudit=?, catatan_audit=?, nama_petugas_unit=?, tanda_tangan_petugas=? WHERE id=?");
          mysqli_stmt_bind_param($stmt, "sssssi", $tanggal, $ruangan, $catatan, $petugas, $ttd, $id);
          mysqli_stmt_execute($stmt);

          mysqli_query($conn, "DELETE FROM detail_audit_etika_batuk WHERE audit_id = $id");
...
<head>
  ...
  <title>Edit Audit Etika Batuk</title>
...
<body class="audit-page">
  <div class="layout">
    ...
    <main>
      ...
      <div class="page-wrap">
        <div class="hero-card">
          <h1>Edit Audit Etika Batuk #<?= (int) $audit['id'] ?></h1>
        ...
        <form method="post">
          ...
          <div class="section-card">
            ...
            <div class="btn-row">
              ...
              <a class="btn btn-secondary" href="../audit_etika_batuk.php?tab=tab-data">Data Audit</a>
            </div>
          </div>
        </form>
      </div>
