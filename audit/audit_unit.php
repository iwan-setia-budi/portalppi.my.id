...
$pageTitle = 'AUDIT UNIT';
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_unit_ok'])) {
  unset($_SESSION['flash_audit_unit_ok']);
  $message = '<div class="info-box success">Data audit unit berhasil disimpan.</div>';
}
...
if (isset($_POST['simpan'])) {
  ...
  if (
    ...
    !$namaPetugasUnit ||
    $ruanganDiaudit === '' ||
    !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)
  ) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_unit/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_unit/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_unit (tanggal_audit, ruangan_diaudit, nama_pasien, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_unit (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_unit_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'au_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_unit/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_unit_ok'] = true;
      header('Location: audit_unit.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit unit.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_unit a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_unit a
  LEFT JOIN detail_audit_unit d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$filterRekapSubbab = isset($_GET['rekap_subbab']) ? (string) $_GET['rekap_subbab'] : '';
$subbabKeys = array_keys($checklistSections);
if ($filterRekapSubbab !== '' && !in_array($filterRekapSubbab, $subbabKeys, true)) {
  $filterRekapSubbab = '';
}

$filterRekapRuangan = isset($_GET['rekap_ruangan']) ? (string) $_GET['rekap_ruangan'] : '';
if ($filterRekapRuangan !== '' && !in_array($filterRekapRuangan, $ruanganDiauditOptions, true)) {
  $filterRekapRuangan = '';
}
...
if ($filterRekapRuangan !== '') {
  $rekapWhere[] = "a.ruangan_diaudit = '" . mysqli_real_escape_string($conn, $filterRekapRuangan) . "'";
}
if ($filterRekapSubbab !== '') {
  $rekapWhere[] = "d.kode_bagian = '" . mysqli_real_escape_string($conn, $filterRekapSubbab) . "'";
}
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_unit a
  JOIN detail_audit_unit d ON a.id = d.audit_id
  $rekapWhereSql
...
foreach ($checklistSections as $kb => $sec) {
  foreach (($sec['items'] ?? []) as $idx => $_txt) {
    $u = $idx + 1;
    $bagianLabelsExport[$kb . str_pad((string) $u, 2, '0', STR_PAD_LEFT)] = (string) $_txt;
  }
}
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_unit_' . $downloadType . '_' . $periodeLabel . '.csv"');
  ...
  if ($downloadType === 'bagian') {
    ...
    while ($row = mysqli_fetch_assoc($qExportBagian)) {
      ...
      fputcsv($output, [
        ...
        $kode,
        $row['item_text'] ?? ($bagianLabelsExport[$kode] ?? 'Bagian Audit'),
        $num,
        $den,
        $pct
      ]);
    ...
    if ($filterRekapSubbab !== '') {
      $matrixWhere[] = "d.kode_bagian = '" . mysqli_real_escape_string($conn, $filterRekapSubbab) . "'";
    }
    if ($filterRekapRuangan !== '') {
      $matrixWhere[] = "a.ruangan_diaudit = '" . mysqli_real_escape_string($conn, $filterRekapRuangan) . "'";
    }
...
<head>
  ...
  <title>Audit Unit | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Unit</h1>
          <p class="subtitle">Form checklist audit unit (sub bab A01–A13), data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_unit/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_unit/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_unit/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_unit/tab_form_audit.php';
            break;
        }
        ?>
      </div>
