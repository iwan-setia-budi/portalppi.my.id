...
$pageTitle = "AUDIT ETIKA BATUK";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_etika_batuk_ok'])) {
  unset($_SESSION['flash_audit_etika_batuk_ok']);
  $message = '<div class="info-box success">Data audit etika batuk berhasil disimpan.</div>';
}
...
$checklistSections = [
  'F' => [
    'title' => 'ETIKA BATUK',
    'items' => [
      'Apakah staf memahami dengan baik cara etika batuk dan bersih',
      'Apakah staf dapat mempraktikkan cara etika batuk dengan baik dan benar',
      'Apakah staf mengetahui jika setelah batuk/bersin menggunakan masker, masker wajib diganti',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit || $ruanganDiaudit === '' || !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_etika_batuk/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_etika_batuk/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_etika_batuk (tanggal_audit, ruangan_diaudit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_etika_batuk (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_etika_batuk_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'ebt_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_etika_batuk/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_etika_batuk_ok'] = true;
      header('Location: audit_etika_batuk.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit etika batuk.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_etika_batuk a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_etika_batuk a
  LEFT JOIN detail_audit_etika_batuk d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_etika_batuk a
  JOIN detail_audit_etika_batuk d ON a.id = d.audit_id
  $rekapWhereSql
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_etika_batuk_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Etika Batuk | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Etika Batuk</h1>
          <p class="subtitle">Form checklist indikator F0001-F0003 etika batuk, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_etika_batuk/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_etika_batuk/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_etika_batuk/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_etika_batuk/tab_form_audit.php';
            break;
        }
        ?>
      </div>
