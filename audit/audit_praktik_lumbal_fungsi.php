...
$pageTitle = "AUDIT PRAKTIK LUMBAL FUNGSI";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_praktik_lumbal_fungsi_ok'])) {
  unset($_SESSION['flash_audit_praktik_lumbal_fungsi_ok']);
  $message = '<div class="info-box success">Data audit praktik lumbal fungsi berhasil disimpan.</div>';
}
...
$checklistSections = [
  'A' => [
    'title' => 'HAND HYGIENE',
    'items' => [
      'Melakukan hand hygiene sebelum kontak pasien',
      'Melakukan hand hygiene sebelum tindakan aseptik',
      'Melakukan hand hygiene setelah kontak pasien',
      'Metode hand hygiene sesuai standar (handrub / sabun)',
    ]
  ],
  'B' => [
    ...
    'items' => [
      ...
      'Menggunakan masker selama tindakan',
      'Menggunakan APD tambahan sesuai indikasi (gaun/face shield)',
    ]
  ],
  'C' => [
    'title' => 'TEKNIK ASEPTIK',
    'items' => [
      'Menggunakan antiseptik yang sesuai (chlorhexidine/povidone iodine)',
      'Antiseptik diaplikasikan dengan teknik yang benar',
      'Waktu kontak antiseptik adekuat dan dibiarkan kering',
      'Tidak menyentuh area steril setelah desinfeksi',
    ]
  ],
  'D' => [
    'title' => 'PROSEDUR STERIL',
    'items' => [
      'Menggunakan alat steril sekali pakai / steril',
      'Menggunakan duk steril',
      'Tidak terjadi break in aseptic technique selama prosedur',
    ]
  ],
  'E' => [
    'title' => 'KESELAMATAN ALAT TAJAM',
    'items' => [
      'Jarum tidak dilakukan recapping',
      'Jarum langsung dibuang ke safety box',
      'Safety box tidak overfill',
    ]
  ],
  'F' => [
    'title' => 'MANAJEMEN LIMBAH',
    'items' => [
      'Limbah medis dibuang sesuai kategori (infeksius/tajam)',
      'Tidak terjadi paparan limbah ke petugas',
    ]
  ],
  'G' => [
    'title' => 'LINGKUNGAN',
    'items' => [
      'Area tindakan bersih',
      'Permukaan kerja telah didesinfeksi sebelum tindakan',
    ]
  ],
  'H' => [
    'title' => 'INSIDEN PPI',
    'items' => [
      'Tidak terjadi needle stick injury',
      'Tidak terjadi kontaminasi selama prosedur',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit || $ruanganDiaudit === '' || !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_praktik_lumbal_fungsi/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_praktik_lumbal_fungsi/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_praktik_lumbal_fungsi (tanggal_audit, ruangan_diaudit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_praktik_lumbal_fungsi (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_praktik_lumbal_fungsi_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'kwt_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_praktik_lumbal_fungsi/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_praktik_lumbal_fungsi_ok'] = true;
      ...
      $message = '<div class="info-box error">Gagal menyimpan data audit praktik lumbal fungsi.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_praktik_lumbal_fungsi a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_praktik_lumbal_fungsi a
  LEFT JOIN detail_audit_praktik_lumbal_fungsi d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_praktik_lumbal_fungsi a
  JOIN detail_audit_praktik_lumbal_fungsi d ON a.id = d.audit_id
  $rekapWhereSql
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_praktik_lumbal_fungsi_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Praktik Lumbal Fungsi | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Praktik Lumbal Fungsi</h1>
          <p class="subtitle">Form checklist indikator A-H praktik lumbal fungsi, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_praktik_lumbal_fungsi/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_praktik_lumbal_fungsi/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_praktik_lumbal_fungsi/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_praktik_lumbal_fungsi/tab_form_audit.php';
            break;
        }
        ?>
      </div>
