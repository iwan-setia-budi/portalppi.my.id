...
$message = '';
if (!empty($_SESSION['flash_audit_tps_ok'])) {
  unset($_SESSION['flash_audit_tps_ok']);
  $message = '<div class="info-box success">Data audit TPS berhasil disimpan.</div>';
}
...
$checklistSections = [
  'G01' => [
    'title' => 'Kebersihan Ruangan',
    'items' => [
      'Area tempat penyimpanan limbah bersih',
      'Plastik limbah terikat',
      'Ada ceklist pembersihan TPS',
      'Suhu dan kelembaban sesuai dan di dokumentasi',
  ...
  'G02' => [
    ...
    'items' => [
      ...
      'Wastafel cuci tangan tampak bersih',
      'Eye wash berfungsi dengan baik',
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_tps/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_tps/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_tps (tanggal_audit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_tps (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_tps_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'tps_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_tps/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_tps_ok'] = true;
      header('Location: audit_tps.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit TPS.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_tps a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_tps a
  LEFT JOIN detail_audit_tps d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_tps a
  JOIN detail_audit_tps d ON a.id = d.audit_id
  $rekapWhereSql
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_tps_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit TPS | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit TPS</h1>
          <p class="subtitle">Form checklist G01–G02 (G0101–G0205), data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_tps/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_tps/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_tps/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_tps/tab_form_audit.php';
            break;
        }
        ?>
      </div>
