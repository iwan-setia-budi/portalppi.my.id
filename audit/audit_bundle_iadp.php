...
$pageTitle = "AUDIT BUNDLE IADP";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_bundle_iadp_ok'])) {
  unset($_SESSION['flash_audit_bundle_iadp_ok']);
  $message = '<div class="info-box success">Data audit bundle IADP berhasil disimpan.</div>';
}
...
$checklistSections = [
  'Z' => [
    ...
    'items' => [
      ...
      'Port kateter selalu didisinfeksi menggunakan alcohol swab sebelum digunakan',
      'Dressing terawat bersih',
      'Ganti dresing secara aspetik',
...
if (isset($_POST['simpan'])) {
  ...
  if (
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_bundle_iadp/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_bundle_iadp/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_bundle_iadp (tanggal_audit, ruangan_diaudit, nama_pasien, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_bundle_iadp (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_bundle_iadp_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'biadp_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_bundle_iadp/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_bundle_iadp_ok'] = true;
      header('Location: audit_bundle_iadp.php?tab=tab-data');
      exit;
    } catch (Throwable $e) {
      mysqli_rollback($conn);
      $dbErr = mysqli_error($conn);
      $hint = trim($dbErr !== '' ? $dbErr : $e->getMessage());
      $message = '<div class="info-box error">Gagal menyimpan data audit bundle IADP.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_bundle_iadp a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  LEFT JOIN detail_audit_bundle_iadp d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_bundle_iadp_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Bundle IADP | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Bundle IADP</h1>
          <p class="subtitle">Form checklist indikator Z0001-Z0005 bundle IADP, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_bundle_iadp/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_bundle_iadp/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_bundle_iadp/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_bundle_iadp/tab_form_audit.php';
            break;
        }
        ?>
      </div>
