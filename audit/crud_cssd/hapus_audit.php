...
ppi_require_admin_delete_redirect('../audit_cssd.php?tab=tab-data');

mysqli_begin_transaction($conn);
try {
  mysqli_query($conn, "DELETE FROM audit_cssd_foto WHERE audit_id = $id");
  mysqli_query($conn, "DELETE FROM detail_audit_cssd WHERE audit_id = $id");
  mysqli_query($conn, "DELETE FROM audit_cssd WHERE id = $id");
  mysqli_commit($conn);
} catch (Throwable $e) {
