...
$pageTitle = "AUDIT TARGETED CONTINUOUS MEDICAL MASK USE";
$activeTab = $_GET['tab'] ?? 'tab-form';
$message = '';
if (!empty($_SESSION['flash_audit_targeted_continuous_medical_mask_use_ok'])) {
  unset($_SESSION['flash_audit_targeted_continuous_medical_mask_use_ok']);
  $message = '<div class="info-box success">Data audit targeted continuous medical mask use berhasil disimpan.</div>';
}
...
$checklistSections = [
  'M' => [
    'title' => 'AUDIT TARGETED CONTINUOUS MEDICAL MASK USE',
    'items' => [
      'Menggunakan masker bedah',
      'Penggunaan masker bedah sesuai',
      'Masker bedah digunakan terus-menerus',
    ]
  ],
];
...
if (isset($_POST['simpan'])) {
  ...
  if (!$tanggalAudit || !$namaPetugasUnit || $ruanganDiaudit === '' || !in_array($ruanganDiaudit, $ruanganDiauditOptions, true)) {
    ...
    try {
      ...
      $uploadDir = __DIR__ . '/../uploads/audit_targeted_continuous_medical_mask_use/';
      if (!is_dir($uploadDir)) {
      ...
      $tandaTanganPetugas = 'uploads/audit_targeted_continuous_medical_mask_use/' . $signatureFileName;

      $stmt = mysqli_prepare($conn, "INSERT INTO audit_targeted_continuous_medical_mask_use (tanggal_audit, ruangan_diaudit, catatan_audit, nama_petugas_unit, tanda_tangan_petugas) VALUES (?, ?, ?, ?, ?)");
      ...
      $stmtDetail = mysqli_prepare($conn, "INSERT INTO detail_audit_targeted_continuous_medical_mask_use (audit_id, kode_bagian, urutan_item, item_text, jawaban) VALUES (?, ?, ?, ?, ?)");
      ...
      if (!empty($_FILES['dokumentasi_foto']['name'][0])) {
        ...
        $stmtFoto = mysqli_prepare($conn, "INSERT INTO audit_targeted_continuous_medical_mask_use_foto (audit_id, nama_file, path_file, ukuran_file) VALUES (?, ?, ?, ?)");
        ...
        for ($i = 0; $i < $jumlahFile; $i++) {
          ...
          $newName = 'tcmmu_' . $auditId . '_' . time() . '_' . $i . '.' . $ext;
          $target = $uploadDir . $newName;
          if (move_uploaded_file($tmp, $target)) {
            $relativePath = 'uploads/audit_targeted_continuous_medical_mask_use/' . $newName;
      ...
      mysqli_commit($conn);
      $_SESSION['flash_audit_targeted_continuous_medical_mask_use_ok'] = true;
      ...
      $message = '<div class="info-box error">Gagal menyimpan data audit targeted continuous medical mask use.'
...
$qTotalData = mysqli_query($conn, "SELECT COUNT(*) AS total FROM audit_targeted_continuous_medical_mask_use a $whereDataSql");
...
$qData = mysqli_query($conn, "
  ...
  FROM audit_targeted_continuous_medical_mask_use a
  LEFT JOIN detail_audit_targeted_continuous_medical_mask_use d ON a.id = d.audit_id
  $whereDataSql
  GROUP BY a.id
...
$qRekapBagian = mysqli_query($conn, "
  SELECT
    ...
    COUNT(*) AS denum
  FROM audit_targeted_continuous_medical_mask_use a
  JOIN detail_audit_targeted_continuous_medical_mask_use d ON a.id = d.audit_id
  $rekapWhereSql
...
$bagianLabelsExport = [
  'M' => 'TARGETED CONTINUOUS MEDICAL MASK USE',
];
...
if (isset($_GET['download_rekap']) && $_GET['download_rekap'] !== '') {
  ...
  header('Content-Disposition: attachment; filename="rekap_audit_targeted_continuous_medical_mask_use_' . $downloadType . '_' . $periodeLabel . '.csv"');
...
<head>
  ...
  <title>Audit Targeted Continuous Medical Mask Use | PPI PHBW</title>
    ...
    <main>
      ...
      <div class="audit-wrapper">
        <section class="hero-header">
          <h1>Audit Targeted Continuous Medical Mask Use</h1>
          <p class="subtitle">Form checklist indikator M0001-M0003 targeted continuous medical mask use, data audit, rekap, dan grafik.</p>
        </section>
        ...
        switch ($activeTab) {
          case 'tab-data':
            include __DIR__ . '/tabs_targeted_continuous_medical_mask_use/tab_data_audit.php';
            break;
          case 'tab-rekap':
            include __DIR__ . '/tabs_targeted_continuous_medical_mask_use/tab_rekap_audit.php';
            break;
          case 'tab-grafik':
            include __DIR__ . '/tabs_targeted_continuous_medical_mask_use/tab_grafik_audit.php';
            break;
          case 'tab-form':
          default:
            include __DIR__ . '/tabs_targeted_continuous_medical_mask_use/tab_form_audit.php';
            break;
        }
        ?>
      </div>
